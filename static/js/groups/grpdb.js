...
function getMemberIds(group_id) {
  return DB.groups({id: group_id}).first()["members"].map(rec => rec.contact_id);
}

function getNonMembers(group_id) {
  let memberIds = getMemberIds(group_id);
  let contactIds = DB.contact_names().map(rec => rec.id);
  let nonMemberIds = contactIds.filter(x => !memberIds.includes(x));
  return DB.contact_names({id: nonMemberIds}).get();
}
